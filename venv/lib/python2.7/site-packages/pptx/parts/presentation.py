...
from .slide import NotesMainPart, SlidePart
...
class PresentationPart(XmlPart):
    ...
    def notes_main(self):
        """
        Return the |NotesMain| object for this presentation. If the
        presentation does not have a notes main, one is created from
        a default template. The same single instance is returned on each
        call.
        """
        return self.notes_main_part.notes_main

    @lazyproperty
    def notes_main_part(self):
        """
        Return the |NotesMainPart| object for this presentation. If the
        presentation does not have a notes main, one is created from
        a default template. The same single instance is returned on each
        call.
        """
        try:
            ...
            notes_main_part = NotesMainPart.create_default(self.package)
            self.relate_to(notes_main_part, RT.NOTES_MASTER)
    ...
    def related_slide_main(self, rId):
        """
        Return the |SlideMain| object for the related |SlideMainPart|
        corresponding to relationship key *rId*.
        """
        return self.related_parts[rId].slide_main
