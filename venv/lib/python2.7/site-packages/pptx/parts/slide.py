...
from ..oxml.slide import CT_NotesMain, CT_NotesSlide, CT_Slide
from ..oxml.theme import CT_OfficeStyleSheet
from ..slide import NotesMain, NotesSlide, Slide, SlideLayout, SlideMain
from ..util import lazyproperty


class BaseSlidePart(XmlPart):
    """
    Base class for slide parts, e.g. slide, slideLayout, slideMain,
    notesSlide, notesMain, and handoutMain.
    """
...
class NotesMainPart(BaseSlidePart):
    """
    Notes main part. Corresponds to package file
    `ppt/notesMains/notesMain1.xml`.
    """
    @classmethod
    def create_default(cls, package):
        """
        Create and return a default notes main part, including creating the
        new theme it requires.
        """
        notes_main_part = cls._new(package)
        theme_part = cls._new_theme_part(package)
        notes_main_part.relate_to(theme_part, RT.THEME)
        return notes_main_part

    @lazyproperty
    def notes_main(self):
        """
        Return the |NotesMain| object that proxies this notes main part.
        """
        return NotesMain(self._element, self)

    @classmethod
    def _new(cls, package):
        """
        Create and return a standalone, default notes main part based on
        the built-in template (without any related parts, such as theme).
        """
        partname = PackURI('/ppt/notesMains/notesMain1.xml')
        content_type = CT.PML_NOTES_MASTER
        notesMain = CT_NotesMain.new_default()
        return NotesMainPart(partname, content_type, notesMain, package)
...
class NotesSlidePart(BaseSlidePart):
    ...
    def new(cls, package, slide_part):
        """
        Create and return a new notes slide part based on the notes main
        and related to both the notes main part and *slide_part*. If no
        notes main is present, create one based on the default template.
        """
        notes_main_part = package.presentation_part.notes_main_part
        notes_slide_part = cls._add_notes_slide_part(
            package, slide_part, notes_main_part
        )
        notes_slide = notes_slide_part.notes_slide
        notes_slide.clone_main_placeholders(notes_main_part.notes_main)
        return notes_slide_part

    @lazyproperty
    def notes_main(self):
        """
        Return the |NotesMain| object this notes slide inherits from.
        """
        notes_main_part = self.part_related_by(RT.NOTES_MASTER)
        return notes_main_part.notes_main
    ...
    @classmethod
    def _add_notes_slide_part(cls, package, slide_part, notes_main_part):
        """
        ...
        notes_slide_part.relate_to(notes_main_part, RT.NOTES_MASTER)
...
class SlideLayoutPart(BaseSlidePart):
    ...
    def slide_main(self):
        ...
        """
        return self.part_related_by(RT.SLIDE_MASTER).slide_main


class SlideMainPart(BaseSlidePart):
    """
    Slide main part. Corresponds to package files
    ppt/slideMains/slideMain[1-9][0-9]*.xml.
    """
    ...
    def slide_main(self):
        """
        The |SlideMain| object representing this part.
        """
        return SlideMain(self._element, self)
