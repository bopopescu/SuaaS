...
class CT_Presentation(BaseOxmlElement):
    ...
    """
    sldMainIdLst = ZeroOrOne('p:sldMainIdLst', successors=(
        'p:notesMainIdLst', 'p:handoutMainIdLst', 'p:sldIdLst',
        'p:sldSz', 'p:notesSz'
    ))
...
class CT_SlideMainIdList(BaseOxmlElement):
    """
    ``<p:sldMainIdLst>`` element, child of ``<p:presentation>`` containing
    references to the slide mains that belong to the presentation.
    """
    sldMainId = ZeroOrMore('p:sldMainId')


class CT_SlideMainIdListEntry(BaseOxmlElement):
    """
    ``<p:sldMainId>`` element, child of ``<p:sldMainIdLst>`` containing
    a reference to a slide main.
    """
    rId = RequiredAttribute('r:id', XsdString)
