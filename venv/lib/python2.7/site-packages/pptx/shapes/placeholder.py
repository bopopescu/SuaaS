...
a slide, layout, or main. Hence there is a non-trivial class inheritance
structure.
"""
...
class _InheritsDimensions(object):
    ...
    def _base_placeholder(self):
        """
        Return the layout or main placeholder shape this placeholder
...
class BasePlaceholder(Shape):
    ...
    behaviors of placeholders on a main, layout, and slide.
    """
    @property
    def idx(self):
        """
...
class LayoutPlaceholder(_InheritsDimensions, Shape):
    ...
    from the main placeholder having the same type, when a matching one
    exists.
    """
    @property
    def _base_placeholder(self):
        """
        Return the main placeholder this layout placeholder inherits from.
        """
        base_ph_type = {
        ...
        slide_main = self.part.slide_main
        return slide_main.placeholders.get(base_ph_type, None)


class MainPlaceholder(BasePlaceholder):
    """
    Placeholder shape on a slide main.
    """
...
class NotesSlidePlaceholder(_InheritsDimensions, Shape):
    ...
    placeholder on the notes main that has the same type (e.g. 'body').
    """
    @property
    def _base_placeholder(self):
        """
        Return the notes main placeholder this notes slide placeholder
        inherits from, or |None| if no placeholder of the matching type is
        present.
        """
        notes_main = self.part.notes_main
        ph_type = self.element.ph_type
        return notes_main.placeholders.get(ph_type=ph_type)
