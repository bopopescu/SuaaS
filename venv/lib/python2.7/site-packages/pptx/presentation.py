...
from .slide import SlideMains, Slides
...
class Presentation(PartElementProxy):
    ...
    __slots__ = ('_slide_mains', '_slides')
    ...
    def notes_main(self):
        """
        Instance of |NotesMain| for this presentation. If the presentation
        does not have a notes main, one is created from a default template
        and returned. The same single instance is returned on each call.
        """
        return self.part.notes_main
    ...
    def slide_layouts(self):
        ...
        |SlideMain| of this presentation. A presentation can have more than
        one slide main and each main will have its own set of layouts.
        This property is a convenience for the common case where the
        presentation has only a single slide main.
        """
        return self.slide_mains[0].slide_layouts

    @property
    def slide_main(self):
        """
        First |SlideMain| object belonging to this presentation. Typically,
        presentations have only a single slide main. This property provides
        simpler access in that common case.
        """
        return self.slide_mains[0]

    @lazyproperty
    def slide_mains(self):
        """
        Sequence of |SlideMain| objects belonging to this presentation
        """
        return SlideMains(self._element.get_or_add_sldMainIdLst(), self)
